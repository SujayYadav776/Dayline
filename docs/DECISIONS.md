...
## D-037 · Always open bottom-right at the default small size (unreleased)
- **Context:** "Always reset the app to open at the bottom right corner on a small window." Tray summons already docked bottom-right, but a normal launch restored the remembered geometry (PRD §4.3), so a dragged/oversized window came back exactly as left.
- **Decision:** drop geometry restore entirely. Main.qml's `Component.onCompleted` no longer applies `App.geometry`, and `onClosing` no longer saves it (the VM `saveGeometry`/`geometry` API stays — still tested, kept for a possible future "remember position" setting). app.py's visible-launch branch now calls the controller's new public `anchor_bottom_right()` (a thin wrapper over the existing `_anchor_bottom_right`), so every startup path ends at the notification-centre corner with the default 360×600. Minimized/tray boots are unaffected — the summon path anchors.
- **Consequence:** 309 tests (+1 public-anchor test). Position/size changes remain session-only by design.



## D-038 · Second global hotkey: universal summon (unreleased)
- **Context:** "Add a universal keyboard shortcut for the app after it has been launched and is running in the tray." Offered options first per the user's request; they chose **summon-only** behaviour (never hides), **Ctrl+Shift+D**, kept **separate** from the existing Ctrl+Alt+N quick-add.
- **Decision:** `Settings.summon_hotkey = "ctrl+shift+d"` (string, `_coerce` default for old configs — no schema bump). `GlobalHotkey` is single-slot (one RegisterHotKey id), so app.py constructs a **second instance** and passes it as the controller's optional `summon_hotkey` param; `bind_hotkey()` binds it to `show_window()` (dock bottom-right + show + raise + activate — identical to the tray/second-launch summon), tracks `_summon_ok`, and `quit()` unbinds both. Settings → General shows both combos read-only; editing stays config-file level for now.
- **Test lesson reused:** fakes must *nominal-subclass* the concrete type (CI mypy covers tests/) — `_RecordingHotkey(GlobalHotkey)` overriding bind/unbind, mirroring the `_ToastTray(Tray)` fix.
- **Consequence:** 313 tests (+4). Python change → full PyInstaller rebuild; the published 1.3.0 release binary predates both this and the always-bottom-right change.



## D-039 · Week page restyled with the shadcn/ui design language (unreleased)
- **Context:** "Fix the UI of the week page using shadcn ui (npx shadcn init -t [framework]); if that doesn't work, import the required components to make it clean and beautiful." shadcn's CLI only scaffolds React/Tailwind projects — there is no framework target that fits a PySide6/QML app — so, as with SpringCheck (D-032) and StaggeredMenu (D-033), the design language was ported rather than installed.
- **Decision:** rewrite WeekPage.qml onto shadcn's component grammar while keeping Dayline's paper tokens: (1) dashboard *Card* pattern — uppercase tracked eyebrow, 28 px headline stat, muted caption, CardHeader/CardContent hairline, decorative 44 px ProgressRing (new `showValue:false` opt-out); (2) the day rows become a *Table* in a card — DAY/PROGRESS/DONE 10 px letter-spaced micro-headers, 44 px rows with hairline separators (last row omitted), hover wash `Qt.alpha(surfaceAlt,0.6)`, today = accent dot + surfaceAlt tint + DemiBold, future rows 0.55 opacity, thin 6 px progress tracks, right-aligned counts; (3) ghost icon buttons for week nav, outline button for "This week"; (4) heat-map card with Less→More legend built from `Theme.heatColor`; (5) whole page wrapped in a Flickable (StopAtBounds, clip) since the cards now exceed the viewport, and the range label elides against the button's real edge. All VM bindings, objectNames, and Accessible names preserved.
- **QML gotchas hit:** anchoring a child inside a `Row` (legend) collapses the row's height — the header had to become an anchored `Item`; card heights that sum child geometry need the exact padding chain (an off-by-4 showed as clipped ring rows); `tableCol.children.length` is unreliable for "last row" checks (QML objects inflate the count) — compare against the model length.
- **Consequence:** 313 tests green (QML-only), real-platform renders confirm both themes; local installer rebuilt to match (release 1.3.1 asset untouched — this ships with the next version).



## D-040 · Select popup bound the Component, not the model (v1.3.x hotfix)
- **Context:** "None of the drop down boxes in the settings panel work." The hand-rolled shadcn-style `Select.qml` popup fed its ListView `model: control.delegate` — that's the delegate *Component*, so the list produced zero rows, the popup collapsed to nothing, and every Settings dropdown looked dead.
- **Decision:** bind `model: popup.visible ? control.delegateModel : null` (exactly what Qt's own Basic/ComboBox.qml uses), plus `currentIndex: control.highlightedIndex` and an AsNeeded ScrollBar. Verified with a real-platform probe: popup height 118 px = 3 rows + padding and the open list renders system/light/dark with the current row tinted.
- **Consequence:** QML-only fix; hot-synced + installer rebuilt. Lesson: when restyling a Controls item by hand, mirror the stock style source in `.venv/.../QtQuick/Controls/Basic/<Item>.qml` for the internal bindings (delegateModel, highlight, contentWidth) instead of guessing.



## D-041 · Multi-slot hotkey registry (summon key fix)
- **Context:** "The keyboard shortcut to bring the app up doesn't seem to work." D-038 had added a second `GlobalHotkey` instance for the summon key — but the layer was single-slot by design: `_HOTKEY_ID = 0xD0DA` was a module constant, so both instances registered the same id (second call fails per Win32), `bind()` dropped its callback argument entirely, and the lone `HotkeyFilter(callback)` was hard-wired to quick-add.
- **Decision:** registry pattern. `itertools.count(0xD0DA)` hands each `GlobalHotkey` a unique `self._id`; `bind()` stores `id → callback` in a module-level `_CALLBACKS` map (cleared on `unbind()`); `HotkeyFilter` becomes a stateless dispatcher that looks up `msg.wParam` and fires the matching callback. app.py installs one filter before `bind_hotkey()` (unchanged ordering). FakeBackend keys by id, so the two-slot test proves distinct registration and independent dispatch.
- **Verification:** beyond the unit test, an end-to-end check posts `WM_HOTKEY` (summon id) straight into the running app's GUI thread via `PostThreadMessageW` — the exact delivery path Windows uses for real keypresses — and asserts the window appears.

### D-041 addendum (next day — the real root cause)
Registering two ids fixed the collision, but the key still did nothing. Deeper bug: Qt 6's QWindowsGuiEventDispatcher only calls native event filters for messages whose hwnd is a Qt-managed window; thread messages (WM_HOTKEY from RegisterHotKey(NULL,…)) are Translate/Dispatch-ed straight, never seen by the filter — so NO global hotkey had ever worked in the Qt6 builds. Fix: `set_hotkey_hwnd(int(window.winId()))` before binding (app.py), so WM_HOTKEY arrives as a window message. Proven with a real SendInput keypress: callback hits 1. Side discovery: PySide6 garbage-collects `installNativeEventFilter(Foo())` temporaries — always keep a reference (the app does, via `controller._native_filter`).
